...
parser <- add_option(
    ...
    type = "character",
    action = "store",
    help = ".RData file with trained model (N)"
)
...
parser <- add_option(
    ...
    type = "character",
    action = "store",
    help = ".RData file with trained model (selfing rate)"
)
...
#### testing
## args$modelFileN <- "caretmodels/modelN202309241639325lSSg.RData"
## args$modelFileSelf <- "caretmodels/model202309241639325lSSg.RData"
## args$fisFile <- "thal/test4GATK_AC.DE.vcftools.vcf.gz.het"
## args$tajimaFile <- "thal/test4GATK_AC.DE.vcftools.vcf.gz.Tajima.D"
## args$rohFile <- "thal/test4GATK_AC.vcf.gz.roh.txt"
## args$chromosomeLengths <- "thal/TAIR10.fa.lengths"
## args$out <- "thal/selfrates_thalianaDE_Ntesting.txt"
...
cat("Starting N-model.\n")
predN <- predict(modelrf,dTest)

#### --- SELFING RATE ---
## Now we can N, we can predict selfing rate
dTest$Ncurrent <- paste(predN)

## -- PREPARE MODELING --
cat("Loading selfing model.\n")
load(args$modelFileSelf)

modelrf <- model[[1]]
modelInfoStr <- model[[2]]

## -- PREDICTION --
cat("Starting selfing prediction.\n")

pred <- predict(modelrf,dTest)

## save output
cat("Saving output: ",args$out,"\n")
out <- bind_cols(fullData,selfRate=pred,N=predN)
